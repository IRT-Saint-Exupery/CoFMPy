...
#
# - `DualMassOscillator.System1.fmu`: This FMU models the first mass and the spring
#   connected to the left wall. It takes as input the force `F` that the second mass
#   applies on the first mass. It outputs the position `x1`, velocity `v1`, and
#   acceleration `a1` of the first mass.
# - `DualMassOscillator.System2.fmu`: This FMU models the second mass and the two other
...
#   of the first mass. It outputs the force `F` that the second mass applies on the
#   first mass.
#
...
# %%
# We now run the simulation for 15 seconds with a 1 ms time step. We retrieve the
...
def plot_results(results):
    plt.plot(results["time"], results[("mass1", "x1")], label="Position")
    plt.plot(results["time"], results[("mass1", "v1")], label="Velocity")
    plt.grid()
...
# with two FMUs. The co-simulation is handled by the Master object, which solves the
# algebraic loop between the two FMUs.
